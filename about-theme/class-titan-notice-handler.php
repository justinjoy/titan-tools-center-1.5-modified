...
class Titan_Notice_Handler {
	...
	public function titan_theme_info_welcome_admin_notice() {
		...
		if ( is_admin() && ! get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) && 'appearance_page_titan-tools-center' !== $current_screen->base ) {
	...
	public function ajax_dismiss_notice() {
		...
		if ( isset( $_POST['type'] ) && 'getting_started_notice' === $_POST['type'] ) {
			add_user_meta( get_current_user_id(), 'gs_notice_dismissed', 'true' );
		}
		wp_die();
	}
				...
				<div class="theme-detail">
					<div class="titan-tools-center-admin-card-header-1">
						<div class="titan-tools-center-header-left">
							<h2>
								<?php echo esc_html( $titan_tools_center_theme->get( 'Name' ) ); ?> <span><?php echo esc_html( $titan_tools_center_theme->get( 'Version' ) ); ?></span>
							</h2>
							<p>
								<?php
								echo wp_kses_post( apply_filters( 'titan_theme_description', $titan_tools_center_theme->get( 'Description' ) ) );
								?>
							</p>
						</div>
